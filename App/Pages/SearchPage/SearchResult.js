...
let params = {"page":currentPage,"areaCode":'',"industryphyName":'',"key":'',"size":'10',"esdateStart":'起始时间',"esdateEnd":'',"regcapGte":'起始资本',"regcapLte":''};//当前第几页
export default class SearchPage extends Component {
    constructor (props) {
        ...
        this.state = {
            ...
            showFoot:0,
            params:{"pageNum":currentPage,"key":'',"industryphyName":"","areaCode":"","size":'10',"esdateStart":'起始时间',"esdateEnd":'',"regcapGte":'起始资本',"regcapLte":''},
        }
    }
    componentDidMount () {
    ...
    searchIndustrySelect (code,number) {
        ...
        params.industryphyName = code.INDUSTRYPHYNAME;
        console.log(params);
        //this.getCompanyListData()
    };
    //显示排序浮层
    ...
    soatOptionActive(itme,index){
       // alert(index)
        ...
        getSortData = ()=>{
            ...
            // console.log(1);
            if (index === 0){
                soatData = Soat.sort(this.state.listData,'time',index);
            }else if (index === 1){
                soatData = Soat.sort(this.state.listData,'time',index,true);
            }else if (index === 2){
                soatData = Soat.sort(this.state.listData,'money',index);
            }else if (index === 3){
                soatData = Soat.sort(this.state.listData,'money',index,true);
            }
    ...
    getView = ({item}) => {
        ...
        return(<TouchableWithoutFeedback
            onPress={()=>{this.props.navigation.navigate('Company',{companyName:item.ENTNAME,info:this.props.navigation.state.params.info})}}
        >
